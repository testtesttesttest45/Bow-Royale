...
public class SoundManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // === ONLY CREATE AudioSources ONCE HERE ===
    ...
    public void PlayBGMByEnvironment(bool useForest)
    {
        ...
        {
            Debug.Log("[SoundManager] BGM already playing: " + bgmToPlay?.name);
            return; // Already playing the correct BGM
        }

        Debug.Log("[SoundManager] Starting BGM: " + bgmToPlay?.name);
        bgmSource.Stop();
    ...
    public void StopBGM()
    {
        if (bgmSource != null)
        {
            Debug.Log($"[SoundManager] Stopping BGM: {bgmSource.clip?.name}, isPlaying={bgmSource.isPlaying}");
            bgmSource.Stop();
